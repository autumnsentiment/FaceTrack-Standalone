...
An experimental native iOS SwiftUI port is available in [`ios/`](ios/). It mirrors the Android v1.4 core pipeline with AVFoundation camera capture, MediaPipe Tasks Vision Face Landmarker, VRCFT v2 face mapping, calibration controls, and OSC/VMC UDP streaming.

实验性原生 iOS SwiftUI 版本位于 [`ios/`](ios/)。该分区对齐 Android v1.4 核心流程：AVFoundation 摄像头、MediaPipe Tasks Vision Face Landmarker、VRCFT v2 面部参数映射、校准控制以及 OSC/VMC UDP 推流。
